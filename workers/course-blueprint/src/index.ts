...
export default { async fetch(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    const raw = await env.AI.run(MODEL, { messages: [{ role: 'system', content: 'Extract only actual academic course topics from the syllabus. Exclude instructor contact details, office hours, grading percentages, room numbers, attendance policy, submission instructions, dates, and generic administrative headings. Return JSON with topics, each having title and nullable description.' }, { role: 'user', content: source }], response_format: { type: 'json_schema', json_schema: { type: 'object', additionalProperties: false, properties: { topics: { type: 'array', items: { type: 'object', additionalProperties: false, properties: { title: { type: 'string' }, description: { type: ['string', 'null'] } }, required: ['title', 'description'] } } }, required: ['topics'] } }, temperature: 0.1, max_tokens: 4096 });
